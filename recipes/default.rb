...
# create bash_profile
template "#{node[:oracle][:user][:home]}/.bash_profile" do
  action :create
  source 'bash_profile.erb'
end

# create bashrc
template "#{node[:oracle][:user][:home]}/.bashrc" do
  action :create
  source 'bashrc.erb'
end
...
execute 'install and setup rlwrap' do
  command 'wget http://utopia.knoware.nl/~hlub/rlwrap/rlwrap-0.37.tar.gz && tar xfvz rlwrap-0.37.tar.gz && cd rlwrap-0.37 && ./configure && make && make install'
  cwd     '/usr/local/src/'
  not_if  'which rlwrap'
end
